...
| `getCreatorContextDraft` | GET .../creator-studio/context-drafts/{draftId} | `nashir.creator_studio.use` | Session owner or workspace admin only; read-only access must not imply transfer/create authority | YES | Implicit | NO | NO | owner, admin, creator (own session) | authGuard → workspaceContextGuard → **nonDisclosingMembershipCheck** → permissionGuard | MEDIUM — 410 Gone for expired drafts |
...
| `getCreatorTransferDraft` | GET .../creator-studio/transfer-drafts/{transferId} | `nashir.creator_studio.use` | Session owner or workspace admin only; future destination service actor — DEFERRED access is deferred until a service actor class is explicitly defined in a future Auth/RBAC gate; must not expose raw platform tokens or prompt text in payloadSummary | YES | Implicit | NO | NO | owner, admin, creator (own session) | authGuard → workspaceContextGuard → **nonDisclosingMembershipCheck** → permissionGuard | **HIGH** — payloadSummary privacy; service actor access deferred |
...
**Service actor clarification:** `future destination service actor` is not a V1 role class and is not approved in this mapping. Any destination-service read access requires a future Auth/RBAC Fix Gate or Service Actor Gate before OpenAPI YAML patching or backend implementation.
