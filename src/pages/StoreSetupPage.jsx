...
const primarySalesChannelOptions = [
  "الموقع الإلكتروني",
  "Instagram",
  "TikTok",
  "TikTok Shop",
...
const acquisitionPlans = {
  "متجر إلكتروني مستقل": {
    ...
    nextAction: "راجع رابط المتجر وحدد الصفحات التي يجب تحليلها لاحقًا.",
  },
  "متجر على منصة تجارة إلكترونية": {
...
const defaultForm = {
  ...
  storeType: "متجر إلكتروني مستقل",
  primarySalesChannel: "الموقع الإلكتروني",
...
export default function StoreSetupPage({ onCreateCampaign = () => {} }) {
  ...
  const acquisitionPlan = useMemo(() => {
    ...
    return {
      ...
      statusItems: [
        ...
        ["قناة البيع محددة", form.primarySalesChannel || "غير محدد"],
  ...
  }, [form.primarySalesChannel, form.storeType]);

  const strategicPlan = useMemo(() => {
    const safeProducts = Array.isArray(products) ? products : [];
    const preferredChannels = Array.isArray(form.preferredChannels) ? form.preferredChannels : [];
    ...
    const deferredChannels = channelOptions
      ...
      .filter((channel) => detectedSuggestedChannels.includes(channel) || ["YouTube", "Google Ads", "Meta Ads", "Salla"].includes(channel))
      .slice(0, 4);
    ...
    const priorityProducts = safeProducts.slice(0, 4).map((product, index) => {
      ...
      const bestChannel = isVideoReady
        ? (preferredChannels.includes("TikTok") ? "TikTok" : preferredChannels.includes("Instagram") ? "Instagram" : "Instagram")
        : isGift
          ? (preferredChannels.includes("Email") ? "Email" : preferredChannels[0] || "WhatsApp Business")
          : preferredChannels[0] || "Instagram";
    ...
    return {
      ...
      audience: [
        ...
        ["الموقع", form.audienceLocation || form.marketLocation || "غير محدد"],
      ...
      messaging: [
        ...
        ["CTA مقترح", preferredChannels.includes("WhatsApp Business") ? "تواصل معنا" : "تسوق الآن"],
      ],
      risks,
      nextAction,
    };
  }, [channelConnections, collectedData, completion, form, products, readinessIssues.length, storeSource.status]);
          ...
          {step === 1 && (
            <Card>
              ...
              <div className="form-grid">
                ...
                <FieldSelect label="قناة البيع الأساسية" value={form.primarySalesChannel} options={primarySalesChannelOptions} onChange={(value) => update("primarySalesChannel", value)} />
                <ChoiceGroup
                  title="نوع النشاط"
              ...
              <Card className="strategy-card">
                ...
                <div className="strategy-grid">
                  ...
                  <div className="strategy-section next-action-card">
                    ...
                    <Button onClick={onCreateCampaign}><Sparkles size={16} /> إنشاء حملة اختبارية</Button>
                  </div>
                </div>
              </Card>
            </div>
...
function ChannelPlan({ title, channels }) {
  return (
    <div className="channel-plan">
      <strong>{title}</strong>
      <div>
        {channels.length ? channels.map((channel) => <span key={channel}>{channel}</span>) : <span>غير محدد</span>}
      </div>
    </div>
  );
}
...
function TimelineCard({ title, text }) {
  return <div className="timeline-card"><strong>{title}</strong><span>{text}</span></div>;
}
